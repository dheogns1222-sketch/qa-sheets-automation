...
파견 QA 팀의 일일 보고와 TC 진행률, 이슈 현황을 스프레드시트 안에서 자동으로 만드는 Google Apps Script 도구다. 파견 QA로 일하며 같은 보고서를 매일 손으로 만드는 팀을 여러 번 봤다. 그때 겪은 문제를 기억해두고, 계산 로직을 처음부터 코드로 새로 짜고 테스트를 붙인 것이 이 저장소다.
...
예전에 고객사·TL·TE 3계층으로 나뉜 스프레드시트를 운영한 적이 있다. TL 마스터 문서 하나에 IFERROR만 8,000개 넘게 걸려 있었고, 열 하나만 잘못 삽입해도 수식이 통째로 깨졌다. "이 셀이 왜 0%지"를 확인할 방법은 눈으로 보는 것뿐이었다. QA가 만든 도구가 검증이 안 된다는 게 이상했다.

REGEXEXTRACT와 IMPORTRANGE로 짠 수식은 작성자만 고칠 수 있는 채로 남는다. 그게 싫어서 같은 문제를 코드로 다시 풀어본 것이 이 저장소다. 계산 로직만 Sheets API 의존 없이 떼어내면 Node로 테스트할 수 있다는 게 핵심 판단이었다.
