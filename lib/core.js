...
(function (root, factory) {
  ...
  /**
   * 당일 이슈 키 목록을 조립한다 (중복 제거, 상태 필터 선택).
   * 원본: IFERROR(TEXTJOIN(", ", TRUE, UNIQUE(FILTER(Issuelist!B:B,
   *        Issuelist!M:M = TEXT(당일, "YYYY-MM-DD"), Issuelist!G:G = 상태))), "-")
   * @param {Array<{key: *, date: *, status?: *}>} issues 이슈 목록
   * @param {Date|string|number} targetDate 기준 날짜
   * @param {string} [statusFilter] 지정 시 해당 상태만 포함
   * @returns {string} 예: 'PRJ-101, PRJ-103' / 없으면 '-'
   */
  function listDailyIssueKeys(issues, targetDate, statusFilter) {
    var targetKey = toDateKey(targetDate);
    var seen = {};
    var keys = [];
    (issues || []).forEach(function (issue) {
      if (!issue || toDateKey(issue.date) !== targetKey) return;
      if (statusFilter !== undefined && statusFilter !== null) {
        var status = issue.status !== null && issue.status !== undefined ? String(issue.status).trim() : '';
        if (status !== String(statusFilter).trim()) return;
      }
      var key = issue.key !== null && issue.key !== undefined ? String(issue.key).trim() : '';
  ...
  /**
   * 일일 보고 본문 문구를 조립한다.
   * 원본: TE 문서 Daily Report!C7 — CHAR(10) 개행 연결로 인사말부터 결과 요약까지
   *       보고 메일 본문을 수식만으로 완성하던 로직의 코드 판.
   * @param {Object} p
   * @param {string} [p.team] 소속 표기 (예: 'QA 1팀')
   * @param {string} [p.name] 보고자 이름
   * @param {Date|string} p.date 보고 기준일
   * @param {ResultCounts} p.counts 당일 TC 결과 집계
   * @param {string} [p.issueLine] summarizeDailyIssues().text
   * @param {string} [p.issueKeys] listDailyIssueKeys() 결과
   * @returns {string} 개행(\n) 포함 보고 본문
   */
  function buildDailyReportMessage(p) {
    var counts = (p && p.counts) || {};
    var progress = computeProgress(counts);
    var executed = (counts.pass || 0) + (counts.fail || 0) + (counts.block || 0);
    var lines = [
      '안녕하세요.',
      ((p && p.team) ? p.team + ' ' : '') + ((p && p.name) || '') + '입니다.',
      '금일 진행된 업무 대응 내용 및 테스트 진행 결과에 대해 하기와 같이 전달드립니다.',
      '',
      '■ 테스트 진행 결과 (' + toDateKey(p && p.date) + ')',
      ' - 수행: ' + executed + '건'
        + ' (Pass ' + (counts.pass || 0)
        + ' / Fail ' + (counts.fail || 0)
        + ' / Block ' + (counts.block || 0) + ')',
      ' - 진행률: ' + formatPercent(progress.progressRate)
        + ' · 성공률: ' + formatPercent(progress.successRate),
      '■ 금일 등록 이슈: ' + ((p && p.issueLine) || '-')
    ];
    if (p && p.issueKeys && p.issueKeys !== '-') lines.push(' - 이슈 번호: ' + p.issueKeys);
    lines.push('', '감사합니다.');
    return lines.join('\n');
  }
  ...
  return {
    ...
    listDailyIssueKeys: listDailyIssueKeys,
    buildDailyReportMessage: buildDailyReportMessage
  };
});
